...
inline static void gen_rand_all(dsfmt *state);
//inline static void gen_rand_array(w128_t array[], int size);
//inline static uint32_t dSFMT_ini_func1(uint32_t x);
//inline static uint32_t dSFMT_ini_func2(uint32_t x);
...
static void initial_mask(dsfmt *state);
static void period_certification(dsfmt *state);
 ...
 */
inline static void do_recursion(w128_t *r, w128_t *a, w128_t *b, w128_t *c, w128_t *lung) {
    w128_t x;

    lshift128(&x, a, SFMT_SL2);
    r->u[0] = a->u[0] ^ x.u[0] ^ ((b->u[0] >> SFMT_SR1) & SFMT_MSK1) ^ (c->u[0] >> SFMT_SR2) ^ (c->u[0] << SFMT_SL1) ^ lung->u[1];
    r->u[1] = a->u[1] ^ x.u[1] ^ ((b->u[1] >> SFMT_SR1) & SFMT_MSK2) ^ (c->u[1] >> SFMT_SR2) ^ (c->u[1] << SFMT_SL1) ^ lung->u[0];
    r->u[0] &= SFMT_LOW_MASK;
    r->u[1] &= SFMT_LOW_MASK;
    lung->u[0] ^= r->u[0];
    lung->u[1] ^= r->u[1];
...
inline static void gen_rand_all(dsfmt *state) {
    ...
    lung = (state->sfmt)[SFMT_N];
    do_recursion(&((state->sfmt)[0]), &((state->sfmt)[0]), &((state->sfmt)[SFMT_POS1]), &((state->sfmt)[SFMT_N -1]), &lung);
    for (i = 1; i < SFMT_N - SFMT_POS1; i++) {
	    do_recursion(&(state->sfmt)[i], &(state->sfmt)[i], &(state->sfmt)[i + SFMT_POS1], &(state->sfmt)[i - 1], &lung);
    }
    for (; i < SFMT_N; i++) {
	    do_recursion(&(state->sfmt)[i], &(state->sfmt)[i], &(state->sfmt)[i + SFMT_POS1 - SFMT_N], &(state->sfmt)[i - 1], &lung);
    }
    (state->sfmt)[SFMT_N] = lung;
}
 ...
 */
//static uint32_t dSFMT_ini_func1(uint32_t x) {
 ...
 * @return 32-bit integer
 */
//static uint32_t dSFMT_ini_func2(uint32_t x) {
 ...
 * 754 format.
 */
void initial_mask(dsfmt *state) {
    int i;
    uint64_t *psfmt;

    psfmt = &(state->sfmt[0].u[0]);
...
static void period_certification(dsfmt *state) {
    ...
    fix[0] = (((SFMT_HIGH_CONST >> SFMT_SR1) & SFMT_MSK2) ^ (SFMT_HIGH_CONST >> SFMT_SR2)) | SFMT_HIGH_CONST;
    fix[1] = (((SFMT_HIGH_CONST >> SFMT_SR1) & SFMT_MSK1) ^ (SFMT_HIGH_CONST >> SFMT_SR2)) | SFMT_HIGH_CONST;
    fix[0] = fix[0] ^ (SFMT_HIGH_CONST >> (64 - 8 * SFMT_SL2));
    new_lung[0] = (state->sfmt[SFMT_N].u[0]) ^ fix[0];
    new_lung[1] = (state->sfmt[SFMT_N].u[1]) ^ fix[1];
    for (i = 0; i < 2; i++) {
	work = new_lung[i] & (state->sfmt_pcv[i]);
	for (j = 0; j < 52; j++) {
	    inner ^= work & 1;
	    work = work >> 1;
	}
    ...
    for (i = 0; i < 2; i++) {
        work = 1;
        for (j = 0; j < 52; j++) {
            if ((work & (state->sfmt_pcv[i])) != 0) {
                (state->sfmt[SFMT_N].u[i]) ^= work;
                return;
            }
            work = work << 1;
        }
 ...
 */
inline double dsfmt_genrand_close1_open2(dsfmt *state) {
    double r;

    assert(state->is_sfmt_initialized);

    if (state->sfmt_idx >= SFMT_N * 2) {
        gen_rand_all(state);
        state->sfmt_idx = 0;
    }
    r = (state->psfmt64)[(state->sfmt_idx)++];
    return r;
}
 ...
 */
void initSomeValues(dsfmt *state){
    state->psfmt64 = &(state->sfmt)[0].d[0];
    state->is_sfmt_initialized = 0;
    state->sfmt_pcv[0] = SFMT_PCV1;
    state->sfmt_pcv[1] = SFMT_PCV2;
}
void dsfmt_init_gen_rand(dsfmt *state, uint32_t seed) {
    initSomeValues(state);
    int i;
    uint32_t *psfmt;

    psfmt = &(state->sfmt[0].u32[0]);
    psfmt[sfmt_idxof(0)] = seed;
    for (i = 1; i < (SFMT_N + 1) * 4; i++) {
        psfmt[sfmt_idxof(i)] = 1812433253UL 
	    * (psfmt[sfmt_idxof(i - 1)] ^ (psfmt[sfmt_idxof(i - 1)] >> 30)) + i;
    }
    initial_mask(state);
    period_certification(state);
    state->sfmt_idx = SFMT_N64;
    state->is_sfmt_initialized = 1;
}
