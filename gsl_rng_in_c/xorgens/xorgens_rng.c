...
UINT rng_xor4096i(xorgens *state, UINT seed) {
    ...
    */
    ...
    if(seed!=0){            /* We are trying to seed the generator. */
        state->i = -1;      /* i < 0 indicates first call */
    }
    ...
    if ((state->i < 0) || (seed != 0)) { /* Initialisation necessary */

        /* weyl = odd approximation to 2**wlen*(3-sqrt(5))/2. */

        if (UINT32) {
            /* Executes if UINT32 is 1 */
            state->weyl = 0x61c88647;
        }
        else {
            /* Executes if UINT32 is 0 */
            state->weyl = ((((UINT) 0x61c88646) << 16) << 16) + (UINT) 0x80b583eb;
        }
        ...
        for (state->i = r-1, k = 4*r; k > 0; k--) { /* Discard first 4*r results */
            t = (state->x)[state->i = (state->i+1)&(r-1)];   t ^= t<<a;  t ^= t>>b;
            v = (state->x)[(state->i+(r-s))&(r-1)];   v ^= v<<c;  v ^= v>>d;
            (state->x)[state->i] = t^v;
        }
    }
...
UREAL rng_xor4096r(xorgens *state, UINT seed)
...
#define SC32  ((UREAL)1/((UREAL)65536*(UREAL)65536)) 
  ...
  while (res == (UREAL)0)               /* Loop until nonzero result.   */
    ...
    res = (UREAL)(rng_xor4096i(state, seed)>>sr);  /* Discard sr random bits.  */
    seed = (UINT)0;                     /* Zero seed for next time. */
    if (UINT32 && UREAL64)              /* Need another call to xor4096i. */
      res += SC32*(UREAL)rng_xor4096i(state, seed);/* Add low-order 32 bits. */
    }
